...
function Profile() {
  ...
  const getInfo = async () => {
    ...
    let result = await fetch("/users/me", config);
    let info = await result.json();
    let status = await result.status;
    console.log(info, status);
    if (status === 200) {
      let date = info.user.createdAt.split("T")[0];
      let count = 0;
      info.polls.forEach((poll) => {
        count += poll.likes.length;
      });
      console.log(count);
      $("#pname").html(info.user.name);
      $("#bname").html(info.user.name);
      $("#pemail").html(info.user.email);
      $("#pdate").html(date);
    ...
    <>
                ...
                <div className="card mb-3 h-100 ">
                  <div className="card-body pt-5 pb-3 px-5">
                    <div className="row">
                      ...
                      <div className="col-sm-9 text-secondary" id="pname"></div>
                    </div>
                    <hr />
                    <div className="row">
                      <div className="col-sm-3">
      ...
      <Container>
        <div className="mypolls">
          <h2 className="text-center mb-3 font-weight-bold">My Polls</h2>
